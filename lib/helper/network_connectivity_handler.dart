  ...
  Future<bool> _checkMyInitialConnectionState(BuildContext context, {bool allowSnackBar = true}) async {
    try {
      ...
      if (allowSnackBar) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('something went wrong, please try again', style: TextStyle(fontFamily: 'mulish',),),));
      }
      return false;
    }
  }
      ...
      if (isConnectedToNetwork) {
        try {
          ...
          if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
            ...
            if (allowSnackBar) {
              ScaffoldMessenger.of(context).clearSnackBars();
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('unable to connect, please try again', style: TextStyle(fontFamily: 'mulish',),),));
            }
            return false;
          }
        } catch (e) {
            if (allowSnackBar) {
              ScaffoldMessenger.of(context).clearSnackBars();
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('timeout, no internet access', style: TextStyle(fontFamily: 'mulish',),),));
            }
            return false;
        }
      }
        ...
        if (allowSnackBar) {
          ScaffoldMessenger.of(context).clearSnackBars();
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('you are not connected to internet', style: TextStyle(fontFamily: 'mulish',),),));
        }
        return false;
      }
    } else {
